...
export const selectMenu: MenuItem[] = [
  ...
  { label: 'Sky', action: () => window.dispatchEvent(new CustomEvent('select-sky')) },
  { label: 'Remove Background', action: () => window.dispatchEvent(new CustomEvent('remove-background')) },
  { label: 'Color Range...', action: (s) => s.setIsColorRangeDialogOpen?.(true) },
  ...
  { label: 'Transform Selection', action: (s) => s.setIsTransformSelectionDialogOpen?.(true) },
];
